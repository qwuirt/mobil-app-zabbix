          ...
          return SizedBox(
            ...
            child: TextButton(
              onPressed: () async {
                ...
                String listUrlApi = ListUrlApiPreferences.getListUrlApi() ?? '';
                String listUsername =
                    ListUserPreferences.getListUsername() ?? '';
                String listPassword =
                ...
                await UrlApiPreferences.setUrlApi(
                    listUrlApi.split(', ')[index]);
                await PasswordPreferences.setPassword(
                    listPassword.split(', ')[index]);
                await SelectPriorityProblemsPreferences
                    .setSelectPriorityProblems(
  ...
  Widget buildProblemList(List<TriggerResultModel> triggerResult) {
    ...
    notFoundProblem(String str) {
      if (checkNullProblem == 0) {
        countNullProblem++;
        if (countNullProblem == mainProblems.length) {
          countNullProblem = 0;
          return Container(
    ...
    if (mainProblems.isNotEmpty) {
      for (var i in triggerResult) {
        List problems = mainProblems
            .where((item) =>
                item.triggerLastEvent.eventid == i.triggerLastEvent.eventid)
            .toList();
    ...
    String listUrlApi = ListUrlApiPreferences.getListUrlApi() ?? '';
    int servers = listUrlApi.split(', ').length;
          ...
          ListView.builder(
              ...
              itemBuilder: (ctx, pos) {
                return BlocBuilder<ZabbixBloc, ZabbixState>(
                  ...
                  int problemPriority = int.parse(mainProblems[pos].priority);
                  String selectPriority = SelectPriorityProblemsPreferences
                          .getSelectPriorityProblems() ??
                      '0';
                  ...
                  saveData() async {
                    ...
                    localStorage.setString(
                        'hostid', mainProblems[pos].triggerHosts[0].hostid);
                    localStorage.setString(
                        'triggerid', mainProblems[pos].triggerid);
                  }

                  problemManualClose() {
                    if (mainProblems[pos].manual_close == '1') {
                      return Text(
                        '[M]',
                        style: TextStyle(color: Colors.white),
                      );
                  ...
                  problemAcknowledged() {
                    if (mainProblems[pos].triggerLastEvent.acknowledged ==
                        '1') {
                      return Text(
                        '[A]',
                        style: TextStyle(color: Colors.white),
                  ...
                  int dateFromJsonInt =
                      int.parse(mainProblems[pos].triggerLastEvent.clock);
                  var dateIssues = DateTime.fromMillisecondsSinceEpoch(
                      dateFromJsonInt * 1000);
                  ...
                  showAlertDialog() {
                    return showDialog<String>(
                        ...
                        builder: (BuildContext context) => AlertDialog(
                            title: Text(
                                '${mainProblems[pos].triggerHosts[0].name}',
                                style: TextStyle(color: Colors.white)),
                            backgroundColor: Colors.black,
                            content: Container(
